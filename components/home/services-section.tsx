  ...
  return (
    <section className="section-spacing bg-white">
      <div className="container-tight">
              ...
              <Link href={`/services/${service.slug}`} className="block" aria-label={service.name}>
                <div className="relative aspect-[16/9] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <span className="text-[9px] font-semibold uppercase tracking-wider text-gold mb-0.5 block">Decoration Service</span>
              ...
              <div className="p-4">
                <div className="flex flex-wrap gap-1 mb-3">
                  {service.features.slice(0, 3).map((feature) => (
                    <span key={feature} className="px-2 py-0.5 text-[9px] rounded-full bg-forest/8 text-forest font-medium">
                      {feature}
                    </span>
                  ))}
                </div>
                <div className="flex items-center justify-between pt-2.5 border-t border-border/30">
                  <span className="text-sm font-bold text-ink">{formatPrice(service.priceFrom)}<span className="text-[10px] font-normal text-secondary-text"> onwards</span></span>
                  <Button size="sm" variant="outline" asChild className="gap-1 h-7 text-[11px] rounded-full px-3">
        ...
        <div className="mt-10 p-6 md:p-8 rounded-2xl bg-forest-light border border-forest/8">
...
function ServiceStat({ icon: Icon, value, label }: { icon: React.ComponentType<any>; value: string; label: string }) {
  return (
    <div className="p-2">
      <div className="w-9 h-9 mx-auto mb-2 rounded-lg bg-white flex items-center justify-center shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
        <Icon className="w-4 h-4 text-forest" />
      </div>
      <p className="text-lg md:text-xl font-extrabold text-forest tracking-tight">{value}</p>
