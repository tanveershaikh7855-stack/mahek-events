...
export function FeaturesBar() {
  return (
    <div className="bg-forest py-3 md:py-4">
      <div className="container-tight">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-6">
          {FEATURES.map((feature) => (
            <div key={feature.label} className="flex items-center gap-2.5 justify-center">
              <feature.icon className="w-5 h-5 text-gold flex-shrink-0" strokeWidth={2} />
              <div>
                <p className="text-xs md:text-sm font-semibold text-white leading-tight">{feature.label}</p>
                <p className="text-[10px] md:text-xs text-white/60 leading-tight">{feature.sublabel}</p>
              </div>
            </div>
          ))}
        </div>
